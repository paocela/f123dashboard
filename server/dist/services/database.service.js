            ...
            if (gp.has_x2 === 1 && gp.full_race_results_id) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (raceResult[i] && raceResult[i] !== 0) {
                        await client.query('INSERT INTO full_race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4)', [gp.full_race_results_id, raceResult[i], i + 1, raceResult[8] === raceResult[i]]);
                    }
                }
                if (raceDnfResult && raceDnfResult.length > 0) {
                    for (const pilotId of raceDnfResult) {
                        await client.query('INSERT INTO full_race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4)', [gp.full_race_results_id, pilotId, 0, false]);
                    }
                }
            }
                ...
                for (let i = 0; i < 8; i++) {
                    if (raceResult[i] && raceResult[i] !== 0) {
                        await client.query('INSERT INTO race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4)', [gp.race_results_id, raceResult[i], i + 1, raceResult[8] === raceResult[i]]);
                    }
                }
                if (raceDnfResult && raceDnfResult.length > 0) {
                    for (const pilotId of raceDnfResult) {
                        await client.query('INSERT INTO race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4)', [gp.race_results_id, pilotId, 0, false]);
                    }
                }
            }
            ...
            if (hasSprint && gp.sprint_results_id) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (sprintResult[i] && sprintResult[i] !== 0) {
                        await client.query('INSERT INTO sprint_result_entries (sprint_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4)', [gp.sprint_results_id, sprintResult[i], i + 1, sprintResult[8] === sprintResult[i]]);
                    }
                }
                if (sprintDnfResult && sprintDnfResult.length > 0) {
                    for (const pilotId of sprintDnfResult) {
                        await client.query('INSERT INTO sprint_result_entries (sprint_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4)', [gp.sprint_results_id, pilotId, 0, false]);
                    }
                }
            }
            ...
            if (gp.qualifying_results_id) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (qualiResult[i] && qualiResult[i] !== 0) {
                        await client.query('INSERT INTO qualifying_result_entries (qualifying_results_id, pilot_id, position) VALUES ($1, $2, $3)', [gp.qualifying_results_id, qualiResult[i], i + 1]);
                    }
                }
            }
            ...
            if (gp.free_practice_results_id) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (fpResult[i] && fpResult[i] !== 0) {
                        await client.query('INSERT INTO free_practice_result_entries (free_practice_results_id, pilot_id, position) VALUES ($1, $2, $3)', [gp.free_practice_results_id, fpResult[i], i + 1]);
                    }
                }
            }
