...
let PostgresService = (() => {
    ...
    let _classThis;
    var PostgresService = _classThis = class {
        constructor() {
            this.pool = new Pool({
    ...
    `, [seasonId]);
        ...
        async getChampionship(seasonId) {
    ...
    `, [seasonId]);
        ...
        async getCumulativePoints(seasonId) {
            const result = await this.pool.query(`
    WITH latest_season AS (
        SELECT id 
        FROM seasons 
...
ORDER BY date ASC
    `, [seasonId]);
      ...
      `, [seasonId]);
        ...
        async getConstructors(seasonId) {
            const result = await this.pool.query(`
      ...
      `);
        ...
        async getConstructorGrandPrixPoints(seasonId) {
            const result = await this.pool.query(`
      WITH latest_season AS (
        SELECT id FROM seasons ORDER BY start_date DESC LIMIT 1
      )
    ...
    `, [seasonId]);
        ...
        async setGpResult(trackId, hasSprint, raceResult, raceDnfResult, sprintResult, sprintDnfResult, qualiResult, fpResult, seasonId) {
            ...
            try {
                ...
                const gpRes = await client.query('SELECT id, race_results_id, sprint_results_id, qualifying_results_id, free_practice_results_id, full_race_results_id, has_x2 FROM gran_prix WHERE track_id = $1 and season_id = $2', [trackId, seasonId]);
                console.log('[setGpResult] gran_prix row:', gpRes.rows[0]);
                if (gpRes.rowCount === 0)
                    throw new Error('Gran Prix not found');
                ...
                if (gp.has_x2 === 1 && gp.full_race_results_id) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        if (raceResult[i] && raceResult[i] !== 0) {
                            const insRes = await client.query('INSERT INTO full_race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4) RETURNING *', [gp.full_race_results_id, raceResult[i], i + 1, raceResult[8] === raceResult[i]]);
                    ...
                    if (raceDnfResult && raceDnfResult.length > 0) {
                        for (const pilotId of raceDnfResult) {
                            const insRes = await client.query('INSERT INTO full_race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4) RETURNING *', [gp.full_race_results_id, pilotId, 0, false]);
                    ...
                    for (let i = 0; i < 8; i++) {
                        if (raceResult[i] && raceResult[i] !== 0) {
                            const insRes = await client.query('INSERT INTO race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4) RETURNING *', [gp.race_results_id, raceResult[i], i + 1, raceResult[8] === raceResult[i]]);
                            console.log(`[setGpResult] Inserted race_result_entries pos ${i + 1}:`, insRes.rows[0]);
                        }
                    }
                    if (raceDnfResult && raceDnfResult.length > 0) {
                        for (const pilotId of raceDnfResult) {
                            const insRes = await client.query('INSERT INTO race_result_entries (race_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4) RETURNING *', [gp.race_results_id, pilotId, 0, false]);
                ...
                if (hasSprint && gp.sprint_results_id) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        if (sprintResult[i] && sprintResult[i] !== 0) {
                            const insRes = await client.query('INSERT INTO sprint_result_entries (sprint_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4) RETURNING *', [gp.sprint_results_id, sprintResult[i], i + 1, sprintResult[8] === sprintResult[i]]);
                    ...
                    if (sprintDnfResult && sprintDnfResult.length > 0) {
                        for (const pilotId of sprintDnfResult) {
                            const insRes = await client.query('INSERT INTO sprint_result_entries (sprint_results_id, pilot_id, position, fast_lap) VALUES ($1, $2, $3, $4) RETURNING *', [gp.sprint_results_id, pilotId, 0, false]);
                ...
                if (gp.qualifying_results_id) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        if (qualiResult[i] && qualiResult[i] !== 0) {
                            const insRes = await client.query('INSERT INTO qualifying_result_entries (qualifying_results_id, pilot_id, position) VALUES ($1, $2, $3) RETURNING *', [gp.qualifying_results_id, qualiResult[i], i + 1]);
                ...
                if (gp.free_practice_results_id) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        if (fpResult[i] && fpResult[i] !== 0) {
                            const insRes = await client.query('INSERT INTO free_practice_result_entries (free_practice_results_id, pilot_id, position) VALUES ($1, $2, $3) RETURNING *', [gp.free_practice_results_id, fpResult[i], i + 1]);
    ...
    __setFunctionName(_classThis, "PostgresService");
    (() => {
